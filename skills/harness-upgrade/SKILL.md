...
2. 读取 `~/.forge/framework/version`（最新版本）
3. 若两者相同 → 提示"已是最新版本"，终止
4. 若 `~/.forge/` 不存在 → 提示：

```
⚠️ 找不到框架本地仓库。
请先安装：curl -sSL https://raw.githubusercontent.com/MOONL0323/forge/main/install.sh | bash
```
...
扫描 `.harness/overrides/` 中的所有文件，与 `~/.forge/` 中对应的新版本模板对比：
...
用户确认后：
1. 将 `~/.forge/framework/` 复制覆盖到 `.harness/framework/`
