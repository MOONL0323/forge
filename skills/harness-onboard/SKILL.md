...
## Step 6.5：Sprint Contract 格式验证门（强制）

> **这是硬性门控：未通过验证的 Plan 不允许进入 Builder 阶段。**

Planner 产出后，读取其输出，逐项验证：

### 验证规则

```
🔍 Sprint Contract 格式验证

  ✓ sprint_contract 字段存在
  ✓ features 数组非空
  ✓ 每个 feature 有 name 和 acceptance_criteria

  对每个 AC：
    ✓ 有 id（格式：AC-1, AC-2...）
    ✓ 有 description（非空，描述具体行为）
    ✓ 有 verifiable_from（值为 ui | api | cli | log 之一）
    ✓ 有 test_method（非空，描述具体验证手段）
    ✓ description 不包含源码引用（grep 检查：import / from src / function( / -> ）

  ✓ blackbox_constraint 字段存在且值为 "✅"
  ✓ 无源码读取类 AC

  验证通过：允许进入 Builder
  验证失败：阻断，返回 Planner 重做
```
...
```
✅ 通过：
  "Click Submit → success modal appears"
  "POST /api/users returns 201 with id field"
  "CLI: ./mytool --version outputs semver string"
  "Log contains 'connection established' after startup"

❌ 不通过（源码引用）：
  "UserService.Create() is called with valid input"
  "the validate() function returns true for valid emails"
  "the db.Update() call includes a timestamp"
  "import path 'pkg/user' is referenced in the handler"
```

### 验证失败时的阻断输出

```
🔴 Sprint Contract 验证失败 — Plan 不允许进入 Builder

  阻断原因：
    ✗ AC-3 的 description 引用了内部实现（"UserService.Create() 被调用"）
    ✗ AC-1 缺少 test_method
    ✗ AC-5 的 verifiable_from 值为 "code"，不是有效值

  请 Planner 重新输出符合格式的 Sprint Contract。
  具体修正要求：
    1. AC 描述必须是外部可观测行为（UI/API/CLI/Log）
    2. 禁止引用函数名、类名、import 路径等内部实现细节
    3. 每个 AC 必须有具体的 test_method
...
快速检测 AC 是否违规（只要匹配以下任一模式即为不通过）：
- 包含 `function(` `()` `->` `.method(` 等代码模式
- 包含 `import ` `from "pkg/` `from './` 等引用模式
- 包含 `src/` `/internal/` `/pkg/` 路径模式
- 描述的是"如何实现"而非"外部表现"
    ...
    "features": [
      {
        ...
        "acceptance_criteria": [
          {
            "id": "AC-1",
            "description": "{specific, externally observable behavior}",
            "verifiable_from": "ui",
            "test_method": "Playwright: click #submit → modal appears"
          },
          {
            "id": "AC-2",
            "description": "{another external behavior}",
            "verifiable_from": "api",
...
通过 Step 6.5 验证后，才允许进入 Builder 阶段。
